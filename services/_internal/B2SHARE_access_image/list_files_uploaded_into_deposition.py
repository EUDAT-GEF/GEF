...
# list_files_uploaded_into_deposition.py is a script that is part of a Docker image solution for accessing the B2SHARE HTTP API.
# It lists all files that have been uploaded into a deposition on the selected B2SHARE instance.
...
try:
    response = requests.get('%s/api/deposition/%s/files' % (target_url, deposition_id), params={'access_token': token}, verify=False)
