...
# list_specific_record.py is a script that is part of a Docker image solution for accessing the B2SHARE HTTP API.
# It lists a specific record that has been uploaded into the selected B2SHARE instance.
...
if (response.status_code == requests.codes.ok):
    ...
    print('That did not work as expected! Server returned HTTP status code %s. Unable to display JSON response.' % response.status_code)